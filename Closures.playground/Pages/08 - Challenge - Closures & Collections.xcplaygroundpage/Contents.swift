//: [Episode 07: compactMap & flatMap](@previous)
//: ## Episode 08: Challenge - Closures & Collections
...
let classLibraryBooksWith_map = students.map { (student) -> [String] in
    student.libraryBooks
}

let classLibraryBooksWith_map_WithShortSyntax = students.map { $0.libraryBooks }

print(classLibraryBooks)
print(classLibraryBooksWith_map)
print(classLibraryBooksWith_map_WithShortSyntax)
...
let classPetsWith_compactMap = students.compactMap { student -> String? in
    student.pet
}

let classPetsWith_compactMap_WithShortSyntax = students.compactMap { $0.pet }

print(classPetsWith_compactMap)
print(classPetsWith_compactMap_WithShortSyntax)
...
*/
let classLibraryBooksWith_flatMap = students.flatMap { (student) in
    student.libraryBooks
}

let classLibraryBooksWith_flatMap_withShortSyntax = classLibraryBooksWith_map_WithShortSyntax.flatMap { $0 }

print(classLibraryBooksWith_flatMap)
print(classLibraryBooksWith_flatMap_withShortSyntax)
//: [Episode 09: filter, reduce, & sort](@next)
